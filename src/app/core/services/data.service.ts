import { INewOrder } from './../../shared/interfaces';
...
export class DataService {
    ...
    getProducts(): Observable<IOrderItem[]> {
        return this.http.get<IOrderItem[]>(this.productsBaseUrl)
            .pipe(
                map(products => {
                    return products;
                }),
    ...
    insertOrder(order: INewOrder): Observable<INewOrder> {
        return this.http.post<INewOrder>(this.ordersBaseUrl, order)
            .pipe(catchError(this.handleError));
    }
        ...
        for (const customer of customers) {
            if (customer && customer.orders) {
                ...
                for (const order of customer.orders) {
                    total += order.totalAmount || order.itemCost;
                }
